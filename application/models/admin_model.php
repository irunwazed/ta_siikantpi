...
class admin_model extends CI_Model {
	...
	$this->db->where('tbl_transaksi_header.status',1);
	$query = $this->db->get();
	if($query->num_rows()>0)
	{
		return $query->num_rows();
	...
	public function hitungPendapatan()
	{
		...
		$this->db->join('tbl_transaksi_header','tbl_transaksi_detail.kode_transaksi=tbl_transaksi_header.kode_transaksi');
		$this->db->where('tbl_transaksi_header.status',1);
		return $this->db->get()->row()->total;
	}
	///akhir home
	public function hitungJumlahTransaksiCari($tgl1,$tgl2)
	{   
	$this->db->select('*');
	$this->db->from('tbl_transaksi_header');
	$this->db->where('tbl_transaksi_header.status',1);
	$this->db->where('tbl_transaksi_header.tanggal_transfer >=',$tgl1);
	$this->db->where('tbl_transaksi_header.tanggal_transfer <=',$tgl2);
	$query = $this->db->get();
	if($query->num_rows()>0)
	{
		return $query->num_rows();
	...
	public function hitungPendapatanCari($tgl1,$tgl2)
	{
		$this->db->select('SUM(harga) as total');
		$this->db->from(' tbl_transaksi_detail');
		$this->db->join('tbl_transaksi_header','tbl_transaksi_detail.kode_transaksi=tbl_transaksi_header.kode_transaksi');
	...
	function ProsesProduk($id_produk){
		return $this->db->query("select a.*,b.nama_brand,c.nama_kategori from tbl_produk a join tbl_brand b on a.brand_id=b.id_brand join tbl_kategori c on a.kategori_id=c.id_kategori where id_produk='$id_produk' ");
	}

	function UpdateProdukLaku($id) {
		return $this->db->query("update tbl_produk set status='0' where id_produk='$id'  ");
	}
	//Akhir Produk
	...
	function GetTransaksi() {
		return $this->db->query("select a.*,b.* from tbl_transaksi_header a join tbl_transaksi_detail b on a.kode_transaksi=b.kode_transaksi where a.status='2' OR a.status='0' order by a.id_transaksi_header desc");
	}
	...
	function ProsesKemasTransaksi($id) {
		return $this->db->query("update tbl_transaksi_header set status='2' where id_transaksi_header='$id'  ");
	}

	function GetTransaksiheader($id) {
		return $this->db->query("select * from tbl_transaksi_header 
		where tbl_transaksi_header.id_transaksi_header='$id' ");
	}
	...
	function GetUpdateTransaksi($jumlah,$kode_produk) {
		$q = $this->db->query("update tbl_produk set stok = stok +'$jumlah'  where kode_produk='$kode_produk'");
		return $q;
	...
	function GetTransaksiSudah() {
		...
		return $this->db->query("select * from tbl_transaksi_header 
		where tbl_transaksi_header.status='1' order by tbl_transaksi_header.id_transaksi_header asc  ");
	...
	function update_Laku($id,$kd)
	{
		$q = $this->db->query("update tbl_produk set laku = laku +'$kd' where kode_produk='$id'");
		$q = $this->db->query("update tbl_produk set status = '1' where kode_produk='$id'");
		return $q;
	}
	function KirimPesanTransaksi($kode_transaksi){
		return $this->db->query("select a.*,b.* from tbl_transaksi_detail a join tbl_transaksi_header b on a.kode_transaksi=b.kode_transaksi where a.kode_transaksi='$kode_transaksi' ");
	}
	function Daftar($kode_transaksi){
		return $this->db->query("select * from tbl_transaksi_detail  where kode_transaksi='$kode_transaksi' order by id_transaksi_detail asc ");
	}
