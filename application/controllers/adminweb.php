...
class adminweb extends CI_Controller
{
	...
	public function home()
	{
		...
		if ($this->session->userdata("logged_in") !== "LoginOke") {
			...
			$data['total_produk'] = $this->admin_model->hitungJumlahProduk();
			$data['total_pendapatan'] = $this->admin_model->hitungPendapatan();
			$data['total_transaksi'] = $this->admin_model->hitungJumlahTransaksi();

			$this->template->load('template', 'adminweb/home', $data);
		}
	}

	public function home_cari()
	{
		...
		if ($this->session->userdata("logged_in") !== "LoginOke") {
			...
			$data['tgl2'] = $tgl2;
			$data['total_produk'] = $this->admin_model->hitungJumlahProduk();
			$data['total_transaksi'] = $this->admin_model->hitungJumlahTransaksiCari($tgl1, $tgl2);
			$data['total_pendapatan'] = $this->admin_model->hitungPendapatanCari($tgl1, $tgl2);

			$this->template->load('template', 'adminweb/home_cari', $data);
		}
	}
	...
	public function produk_simpan()
	{
		...
		$this->form_validation->set_rules('nelayan', 'Nelayan', 'required');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (empty($_FILES['userfile']['name'])) {
				...
				$in_data['nama_nelayan'] = $this->input->post('nama_nelayan');
				$in_data['nelayan'] = $this->input->post('nelayan');
				...
				$in_data['tanggal'] = $this->input->post('tanggal');
	...
	public function produk_edit()
	{
		if ($this->session->userdata("logged_in") !== "LoginOke") {
			...
			foreach ($query->result_array() as $tampil) {
				...
				$data['nama_nelayan'] = $tampil['nama_nelayan'];
				$data['nelayan'] = $tampil['nelayan'];
	...
	public function produk_proses()
	{
		...
		if ($this->session->userdata("logged_in") !== "LoginOke") {
			...
			$id  = $this->uri->segment(3);
			$this->admin_model->UpdateProdukLaku($id);

			$query = $this->admin_model->ProsesProduk($id);
			foreach ($query->result_array() as $tampil) {

				redirect('https://wa.me/' . $tampil['nelayan'] . '?text=Halo%20pak%20' . $tampil['nama_nelayan'] . '%0AJenis Ikan %20*' . $tampil['nama_brand'] . '* %0ALaku Sebanyak %20*' . $tampil['laku'] . '%20.Kg* %0ATotal Harga %20 *Rp.' . $this->cart->format_number($tampil['laku'] * $tampil['harga']) . '* %0A%0A(Admin %20*SMART %20TPI %20 MOROWALI*)%0AWeb %20 http://smart-tpi.morowalikab.go.id/"');
			}
	...
	public function buku_tamu_balas_simpan()
	{
		...
		$this->email->from('amanjabal55@gmail.com', 'Admin SI IKAN TPI');
		$this->email->to($email);
	...
	public function transaksi_kirim()
	{
	...
	public function transaksi_proses()
	{
		...
		if ($this->session->userdata("logged_in") !== "LoginOke") {
			...
			$proses = $this->admin_model->GetDetailTransaksi($kode_transaksi);
			foreach ($proses->result_array() as $tampil) {

				$this->admin_model->update_Laku($tampil['kode_produk'], $tampil['jumlah']);
			}
	...
	public function transaksi_kemas()
	{
		if ($this->session->userdata("logged_in") !== "LoginOke") {
			redirect('adminweb');
		} else {
			$id  = $this->uri->segment(3);
			$this->admin_model->ProsesKemasTransaksi($id);
			$this->session->set_flashdata('berhasil', 'Proses Kemas Berhasil');
			redirect("adminweb/transaksi");
		}
	}
	...
	public function transaksi_hapus()
	{
		...
		$detail = $this->admin_model->GetDetailTransaksi($id);
		foreach ($detail->result_array() as $tampil) {

			$this->admin_model->GetUpdateTransaksi($tampil['jumlah'], $tampil['kode_produk']);
		}
	...
	public function transaksi_KirimPesan()
	{
		...
		if ($this->session->userdata("logged_in") !== "LoginOke") {
			...
			$totalTransaksi = $this->admin_model->GetDetailTotal($kode_transaksi);
			foreach ($totalTransaksi->result_array() as $tampil) {
				$total = $tampil['total'];
			}



			$query = $this->admin_model->KirimPesanTransaksi($kode_transaksi);
			foreach ($query->result_array() as $tampil) {

				redirect('https://wa.me/' . $tampil['no_telepon'] . '?text=Halo%20' . $tampil['penerima'] . '%0ATotal Transaksi Anda %20*Rp.' . $this->cart->format_number($total) . '* %0ABatas Waktu Pembayaran %20*' . $tampil['tanggal_transfer'] . '* %0APembayaran diLakukan di TPI(Tempat Pelelangan Ikan) kelurahan Matano %0A%0A(Admin %20*SMART %20TPI %20 MOROWALI*)%0AWeb %20 http://smart-tpi.morowalikab.go.id"');
			}
		}
	}
}
